...
function provider(): Provider | null {
  const configured = (process.env.COOKING_AGENT_PROVIDER || 'deepseek').toLowerCase();
  const choice = configured === 'chatgpt' ? 'openai' : configured;
...
6. 用户需求下方可能会出现“可信工具报告”，由菜谱库检索和食材核对程序生成。必须把其中的必需项、缺少项、时间状态、选中菜谱和安全基线视为事实，不得自行替换或猜测；不得把缺少项说成已拥有。状态为 missing_requirements、insufficient_time 或 heat_limit 时，必须先明确说明目标无法按原条件完成，再给替代方案；heat_limit 时不得建议超过工具报告上限的档位。
